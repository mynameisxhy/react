...
export default class SaveUpdate extends Component {
      ...
      this.setState({
        options:result.map((item)=>{
          return{
            value:item._id,
            label:item.name,
            isLeaf:false //表示还有下级菜单
          }
        })
      })
    }
  };
  //loadData 实现动态加载 二级分类数据
  ...
  render() {
    ...
    return <Card
      title={<div className="product-title"><Icon type="allow-left" /><span>添加商品</span></div> }>
      <Form {...formItemLayout} onSubmit={this.addProduct}>
        ...
        <Item label="选择分类" wrapperCol={{span:5}} >
          <Cascader options={options} loadData={this.loadData} placeholder="请选择分类" />
        </Item>
        <Item label="商品价格">
          <InputNumber
            //对输入的数据进行格式化
